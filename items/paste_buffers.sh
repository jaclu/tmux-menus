...
static_content() {
    ...
    if $cfg_use_whiptail; then
        ...
        select_cmd="$TMUX_BIN $choose_buffer & $d_items/hints/choose-buffer.sh"
    fi
    ...
    set -- \
        ...
        "$d_items/hints/choose-buffer.sh $f_current_script" \
        0.0 M H "Help                             $nav_next" \
        "$d_items/help_paste_buffers.sh $f_current_script"
