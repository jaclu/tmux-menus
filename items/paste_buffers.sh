...
static_content() {
	...
	select_cmd="$TMUX_BIN $choose_buffer & $d_items/hints/hints_paste_buffers_select.sh"
    fi
    ...
    set -- \
        ...
        0.0 S \
	0.0 M S "Key hints - Select paste buffer  $nav_next" "$d_items/hints/hints_paste_buffers_select.sh $f_current_script" \
