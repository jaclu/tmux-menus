...
dynamic_content() {
    ...
    ${b_all_helpers_sourced:-false} || source_all_helpers "pane_move:dynamic_content()"
...
static_content() {
    set -- \
        ...
        0.0 M H "Help, Move        $nav_next" "$cfg_d_menus/help/h_pane_move.sh $0"
    ...
    set -- \
        0.0 S \
        0.0 C p "Swap Prev" "swap-pane -U $runshell_reload_mnu" \
        0.0 C n "Swap Next" "swap-pane -D $runshell_reload_mnu"
    menu_generate_part 3 "$@"

    set -- \
        0.0 S \
        2.4 E b "Break to new window" "$d_scripts/break_pane.sh ; $0" \
        1.8 E m "Move to other" "$d_scripts/act_choose_tree.sh p m"
