...
generate_content() {
    ...
    #     'kill-pane -a'" \
    #     0.0 S \
    #     0.0 M H 'Help -->' "help_panes.sh $current_script"
...
p_display_menu() {
    ...
    if tmux_vers_compare 2.0; then
        while IFS= read -r line; do
            # log_it "><> line [$line]"
            if [ "$line" = "$zoom_action_placeholder" ]; then
                if [ "$(tmux list-panes | wc -l)" -gt 1 ]; then
                    ...
                    set -- "$@" \
                        "\"<P> $zoom_action pane\"" C "\"resize-pane -Z $menu_reload\""
                fi
                continue
            elif echo "$line" | grep -q 'Zoom'; then
                echo "found it with grep!"
                echo "Line [$line]"
                echo "hint [$zoom_action_placeholder]"
                exit 1
            fi
            set -- "$@" "$line"
        done <"$f_cache_file"
