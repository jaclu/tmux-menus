...
dynamic_content() {
    ...
    [ -n "$vers_no" ] && set -- "$@" 0.0 T "      Version: $vers_no"
    [ -n "$td_vers" ] && set -- "$@" 0.0 T " Vers release: $td_vers"
    [ -n "$branch" ] && set -- "$@" 0.0 T "       Branch: $branch"
    [ -n "$td_pull" ] && set -- "$@" 0.0 T "Latest update: $td_pull"
...
static_content() {
    set -- \
        ...
        0.0 T " $nav_home#[default,nodim]  Back to start menu." \
        0.0 S \
    ...
    ${b_use_alt_handler:-false} || {
        set -- \
            ...
            0.0 T "      no items use either as shortcuts."
        menu_generate_part 2 "$@"
    }

    set -- \
        0.0 T "" \
        0.0 T "#[align=centre]--------  About this plugin  -------"
