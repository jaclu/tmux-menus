...
cache_create_folder() {
    ...
    $cfg_use_cache || error_msg_safe "cache_create_folder() - called when cache is disabled"
    [ -z "$d_cache" ] && {
        error_msg_safe "variable d_cache undefined"
    }

    mkdir -p "$d_cache" || {
        error_msg_safe "Failed to create cache folder: $d_cache"
    }
}
...
cache_clear() { # only cache
    ...
    $cfg_use_cache || error_msg_safe "cache_clear() - called when not using cache"
...
cache_prepare() {
    ...
    $cfg_use_cache || error_msg_safe "cache_prepare() - called when not using cache" 1
...
cache_add_ok_vers() {
    ...
    [ -z "$1" ] && error_msg_safe "cache_add_ok_vers() - no param"
...
cache_add_bad_vers() {
    ...
    [ -z "$1" ] && error_msg_safe "cache_add_bad_vers() - no param"
...
cache_save_known_tmux_versions() { # tmux stuff
    ...
    cat <<EOF >"$f_cache_known_tmux_vers" || error_msg_safe "Failed to save known versions" 1
#!/bin/sh
...
cache_param_write() { # tmux stuff
    ...
    $cfg_use_cache || error_msg_safe "cache_param_write() - called when not using cache"

    cache_prepare

    # need to be in repo base dir for the git chcecks below
    cd "$D_TM_BASE_PATH" || error_msg_safe "Failed to cd into $D_TM_BASE_PATH"
    ...
    f_params_tmp=$(mktemp) || error_msg_safe "Failed to create tmp config file"
